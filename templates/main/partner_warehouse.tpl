        ...
        <table>
            ...
            {foreach from=$ns.products item=product}
                {if isset($ns.productsQuantity[$product->getId()]) && abs($ns.productsQuantity[$product->getId()])>0.01}
                    ...
                    <tr data-id="{$product->getId()}" data-type="product" style="{if $ns.productsQuantity[$product->getId()]<0 or $lastSale->getOrderDateDiffWithNow()>60}color:red;{/if} {if $product->getSalePrice() > 0 && $ns.productLastSellPrice[$product->getId()] >= $product->getSalePrice()} background:yellow;{/if}" >
                        <td>{$product->getId()}</td>
                        ...
                        <td>{$ns.productLastSellPrice[$product->getId()]|number_format:2}</td>  
