...
namespace crm\actions\main\payment {
    ...
    class ExportCsvAction extends BaseAction {
        ...
        public function exportCsv($rows, $startDate, $endDate, $partnerDto = null) {
            ...
            $total = [1=>0, 2=>0];
            foreach ($rows as $row) {
                $total[intval($row->getCurrencyDto()->getId())] += $row->getAmount();
            ...
            foreach ($rows as $row) {
                fputcsv($output, [$row->getDate(), $row->getPartnerDto()->getName(), $row->getAmount(), $row->getCurrencyDto()->getTemplateChar(), $row->getNote()]);
            }
            exit;
        }
