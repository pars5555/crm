...
namespace crm\actions\main\rorder {
    ...
    class UpdateRecipientOrderAction extends BaseAction {
        ...
        public function service() {
            ...
            RecipientOrderManager::getInstance()->updateRecipientOrder($id, $partnerId, $date, $paymentDeadlineDate, $note);
            unset($_SESSION['action_request']);
            $_SESSION['success_message'] = 'Purchase Order Successfully updated!';
            $this->redirect('rorder/' . $id);
        }
        ...
        private function validateFormData() {
            ...
            if (empty(NGS()->args()->order_date)) {
                throw new RedirectException('rorder/edit/' . NGS()->args()->id, "Invalid Date.");
            }

            if (!isset(NGS()->args()->partnerId) || !is_numeric(NGS()->args()->partnerId) || NGS()->args()->partnerId <= 0) {
                throw new RedirectException('rorder/edit/' . NGS()->args()->id, "Invalid Partner.");
            }
            if (empty(NGS()->args()->payment_deadline)) {
                throw new RedirectException('rorder/edit' . NGS()->args()->id, "Invalid Payment Date.");
            }
        }
