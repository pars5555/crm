...
namespace crm\actions\main\rorder {
    ...
    class ExportCsvAction extends BaseAction {
        ...
        public function service() {
            ...
            $rows = RecipientOrderManager::getInstance()->getRecipientOrdersFull($where, 'order_date', "DESC");
        ...
        public function exportCsv($rows, $startDate, $endDate, $partnerDto = null) {
            ...
            foreach ($rows as $row) {
                ...
                $recipientOrderLinesDtos = $row->getRecipientOrderLinesDtos();
                fputcsv($output, ['', 'Product', 'Qty', 'Amount']);
                foreach ($recipientOrderLinesDtos as $recipientOrderLineDto) {
                    $currencyChar = $recipientOrderLineDto->getCurrencyDto()->getTemplateChar();

                    fputcsv($output, ['', $recipientOrderLineDto->getProductDto()->getName(), $recipientOrderLineDto->getQuantity(), $recipientOrderLineDto->getUnitPrice() . ' ' . $currencyChar]);
                }
            }
            exit;
        }
