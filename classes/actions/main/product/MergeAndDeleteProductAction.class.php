...
namespace crm\actions\main\product {
    ...
    class MergeAndDeleteProductAction extends BaseAction {
        ...
        public function service() {
            ...
            if (isset(NGS()->args()->dst_id)) {
                $destinationProductId = NGS()->args()->dst_id;
            } else {
                $_SESSION['error_message'] = 'Destination Product ID is missing';
                $this->redirect('product/list');
            }
            SaleOrderLineManager::getInstance()->replaceProductId($productId, $destinationProductId);
            PurchaseOrderLineManager::getInstance()->replaceProductId($productId, $destinationProductId);
