<?php

/**
 * @author Levon Naghashyan <levon@naghashyan.com>
 * @site http://naghashyan.com
 * @mail levon@naghashyan.com
 * @year 2010-2015
 * @package loads.site.main
 * @version 2.0.0
 */

namespace crm\loads\main\whishlist {

    use crm\loads\AdminLoad;
    use crm\managers\WhishlistManager;
    use NGS;

    class UpdateLoad extends AdminLoad {

        public function load() {
            $this->initErrorMessages();
            $this->initSuccessMessages();
            $id = intval(NGS()->args()->id);
            $whishlist = WhishlistManager::getInstance()->selectByPk($id);
            if ($whishlist) {
                if (!isset($_SESSION['action_request'])) {
                    $_SESSION['action_request'] = [
                        'updated_at' => $whishlist->getUpdatedAt(),
                        'name' => $whishlist->getName(),
                        'target_price' => $whishlist->getTargetPrice(),
                        'asin_list' => $whishlist->getAsinList()
                    ];
                }
                $this->addParam("whishlist", $whishlist);
                $this->addParam('req', $_SESSION['action_request']);
                unset($_SESSION['action_request']);
            }
        }

        public function getTemplate() {
            return NGS()->getTemplateDir() . "/main/whishlist/update.tpl";
        }

    }

}
